...
async def auth_exception_handler(request: Request, exc: LoginRequired) -> RedirectResponse:
    return RedirectResponse(url="/login?next=" + request.url.path, status_code=303)


def add_auth_exception_handler(app: FastAPI) -> None:
    app.add_exception_handler(LoginRequired, auth_exception_handler)  # type: ignore[arg-type]
