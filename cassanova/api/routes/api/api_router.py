...
def get_cassanova_api_router() -> APIRouter:
    ...
    cassanova_api_router.include_router(
        cluster_router, dependencies=[Depends(require_permission("cluster:view"))]
    )
    cassanova_api_router.include_router(tools_router, dependencies=[Depends(require_user)])
    cassanova_api_router.include_router(
        data_router, dependencies=[Depends(require_permission("data:read"))]
    )
    cassanova_api_router.include_router(
        auth_router, dependencies=[Depends(require_permission("cluster:admin"))]
    )
    cassanova_api_router.include_router(
