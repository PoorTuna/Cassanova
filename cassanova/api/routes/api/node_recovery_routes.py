...
def get_k8s_context(request: Request) -> dict[str, Any] | None:
    ...
    return {"core": core, "custom": custom, "enabled": config.k8s.node_recovery.enabled}
...
def get_status_route(ctx: dict[str, Any] | None = Depends(get_k8s_context)) -> dict[str, Any]:
    if not ctx:
...
def approve_route(
    body: ApproveRequest, ctx: dict[str, Any] | None = Depends(get_k8s_context)
) -> dict[str, str]:
    if not ctx:
        raise HTTPException(503, "Kubernetes connection not available")
    ...
    try:
        pod_recovery.approve_recovery(
            ctx["core"], ctx["custom"], body.recovery_id, body.approved_by
        )
...
def cancel_route(
    recovery_id: str, ctx: dict[str, Any] | None = Depends(get_k8s_context)
) -> dict[str, str]:
    if not ctx:
        raise HTTPException(503, "Kubernetes connection not available")
...
def get_recovery_enabled() -> dict[str, bool]:
    config = get_clusters_config()
    return {"enabled": config.k8s.node_recovery.enabled, "k8s_enabled": config.k8s.enabled}
