...
def detect_database_technology(session: Session) -> str:
    cluster_name = str(session.cluster.metadata.cluster_name or id(session.cluster))
...
def _detect_technology(session: Session) -> str:
    try:
