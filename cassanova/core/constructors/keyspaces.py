...
def generate_keyspaces_info(keyspaces: list[tuple[str, KeyspaceMetadata]]) -> list[KeyspaceInfo]:
    return [
        KeyspaceInfo(
            name=name,
            replication=keyspace_metadata.replication_strategy.export_for_schema()
            if keyspace_metadata.replication_strategy
            else None,
            ...
            user_types={
                k: serialize_to_primitive(v) for k, v in keyspace_metadata.user_types.items()
            },
            functions={
                k: serialize_to_primitive(v) for k, v in keyspace_metadata.functions.items()
            },
            aggregates={
                k: serialize_to_primitive(v) for k, v in keyspace_metadata.aggregates.items()
            },
