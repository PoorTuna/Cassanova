...
def get_recovery_status(
    core_api: CoreV1Api, custom_api: CustomObjectsApi, enabled: bool
) -> dict[str, Any]:
    if not enabled:
        return {"jobs": [], "total": 0}
...
def approve_recovery(
    core_api: CoreV1Api, custom_api: CustomObjectsApi, recovery_id: str, approved_by: str
) -> None:
...
def cancel_recovery(custom_api: CustomObjectsApi, recovery_id: str) -> None:
    pod_name = _parse_recovery_id(recovery_id)
...
def _collect_pending_recoveries(core_api: CoreV1Api, recoveries: dict[str, dict[str, Any]]) -> None:
    try:
...
def _collect_active_recoveries(
    custom_api: CustomObjectsApi, recoveries: dict[str, dict[str, Any]]
) -> None:
    try:
...
def _build_status_summary(job_list: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "total": len(job_list),
...
def _fetch_pending_k8ssandra_pods(core_api: CoreV1Api) -> list[Any]:
    resp = core_api.list_pod_for_all_namespaces(
        field_selector="status.phase=Pending",
        label_selector="app.kubernetes.io/managed-by=k8ssandra-operator",
    )
    return resp.items  # type: ignore[no-any-return]


def _fetch_all_k8ssandra_tasks(custom_api: CustomObjectsApi) -> list[dict[str, Any]]:
    try:
        resp = custom_api.list_cluster_custom_object(
            group="control.k8ssandra.io", version="v1alpha1", plural="k8ssandratasks"
        )
...
def _create_replacenode_task(
    ...
    task_body = {
        ...
        "spec": {
            ...
            "template": {"jobs": [{"command": "replacenode", "args": {"pod_name": pod_name}}]},
        },
    }
    ...
    try:
        ...
        logger.info(
            f"Created K8ssandraTask {task_name} for pod {pod_name} (approver: {approved_by})"
        )
...
def _delete_task(custom_api: CustomObjectsApi, task: dict[str, Any]) -> None:
...
def _build_pending_recovery_entry(rec_id: str, pod: Any) -> dict[str, Any]:
    return {
        "id": rec_id,
...
def _build_active_recovery_entry(
    rec_id: str, pod_name: str, task: dict[str, Any]
) -> dict[str, Any]:
    state = "active"
...
def _extract_pod_name_from_task(task: dict[str, Any]) -> str | None:
    try:
        jobs = task.get("spec", {}).get("template", {}).get("jobs", [])
        if not jobs:
            return None
        return jobs[0].get("args", {}).get("pod_name")  # type: ignore[no-any-return]
    except (AttributeError, KeyError):
        return None


def _find_task_by_pod_name(custom_api: CustomObjectsApi, pod_name: str) -> dict[str, Any] | None:
    tasks = _fetch_all_k8ssandra_tasks(custom_api)
    for task in tasks:
...
def _get_pod_or_raise(core_api: CoreV1Api, name: str) -> Any:
    try:
        resp = core_api.list_pod_for_all_namespaces(field_selector=f"metadata.name={name}")
        if resp.items:
            return resp.items[0]
    except ApiException:
        pass
...
def _get_cluster_name_or_raise(pod: Any) -> str:
    cluster_name = pod.metadata.labels.get("k8ssandra.io/cluster-name")
    if not cluster_name:
        raise ValueError("Pod missing cluster-name label")
    return cluster_name  # type: ignore[no-any-return]


def _has_volume_affinity_issue(pod: Any) -> bool:
