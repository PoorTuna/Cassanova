...
class LDAPConfig(BaseModel):
    enabled: bool = False
    server_uri: str = Field("ldap://localhost:389",
                            description="LDAP Server URI (e.g., ldap://hostname:389 or ldaps://hostname:636)")
    bind_dn: Optional[str] = Field(None,
                                   description="DN to bind with for user search (Service Account). If None, anonymous bind is used.")
    ...
    user_search_filter: str = Field("(uid={username})",
                                    description="Filter to find user by username. {username} placeholder will be replaced with input username.")
